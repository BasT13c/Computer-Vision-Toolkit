...
from lib.Noise.salt_and_pepper_noise import salt_pepper_noise
...
from lib.Face.face_recognition import find_faces
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def apply_clustering(self) -> None:
        ...
        if method == 'K-Means':
            ...
            if self.supervised:
                # select seeds
                seeds = segments.selectSeeds(image)
            else: 
                # Unsupervised Region Growing
                # Create seed list based on image's local minima
                seeds = segments.generateSeeds()
            
            # check if seed list is not empty
            if seeds is not None:
                # plot seeds on input image
                self.plot_seeds(seeds)
                print(f"Seeds number: {len(seeds)}")
                # Apply region growing
                self.segmented_image = segments.applyRegionGrow(seeds)          
    ...
    def plot_seeds(self, seeds):
        ...
        self.ui.widget_seg_input = ImagePlotter(seeds, self.image_path, self.colored)
